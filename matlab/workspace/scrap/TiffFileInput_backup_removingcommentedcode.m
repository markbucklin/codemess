classdef (CaseInsensitiveProperties, TruncatedProperties)  TiffFileInput <  ignition.core.Module ...
	...
	properties (SetAccess = ?ignition.core.Module)
		NextFrameIdx = 0
		NextBufferIdx = 0
	end
	properties (SetAccess = ?ignition.core.Module, Transient)
	...
	% todo: VideoSegment -> make FrameData, FrameTime, etc Dependent
	
	% todo: FrameBuffer and/or FrameBufferCollection
	
	% TIFF-STACK -FILE & -FRAME INFO
	properties (SetAccess = ?ignition.core.Module, Nontunable)
		TiffObj(:,1) Tiff
		StackInfo
		FileInfo
		TagIDs
	...
	properties (SetAccess = ?ignition.core.Module, Nontunable, Hidden)
		ParallelFutureObj
		NumBufferBlocks
		PoolTimeoutListener
	end
	...
	properties (SetAccess = ?ignition.core.Module, Hidden)
		StartFrameIdx = 1
	...
	methods (Access = protected)
		...
		function setupImpl(obj)
			fprintf('BufferTiffStackLoader: Setup\n')
			
			if ~obj.IsInitialized
				initializeModuleEnvironment(obj); % @Module
				initialize(obj)
			end
		...
		function varargout = stepImpl(obj)
			...
			if (obj.UseParallel) && (~obj.EnvironmentObj.ParallelPoolObj.Connected)
			...
			vidSegment = ignition.shared.datatype.VideoSegment(frameData, frameTime, frameInfo, frameIdx);
			
			% TODO: ADD VIDEO SEGMENT TO BUFFER... to combine?
			% bufferVideoSegment(obj, vidSegment);
			
			if nargout
				% 				if obj.VideoSegmentOutputPort
				% 					varargout = {vidSegment};
				%
				% 				else
				% RETURN VARIABLE OUTPUT ARGUMENTS
				requestedOutput = [...
					obj.VideoSegmentOutputPort,...
					obj.FrameDataOutputPort,...
					obj.FrameTimeOutputPort,...
					obj.FrameInfoOutputPort,...
					obj.FrameIdxOutputPort
					];
				availableOutput = {vidSegment, frameData, frameTime, frameInfo, frameIdx};
				varargout = availableOutput(requestedOutput);
				% 				end
			end
			
			% TODO: timer -> async
% 			dispatchRemoteLoad(obj)
			loadFcn = @obj.dispatchRemoteLoad;
			ignition.internal.ScheduledTask( loadFcn, 0);
% 			ignition.internal.ScheduledTask( @dispatchRemoteLoad, 0, obj)
			% 			numArgsOut = nargout;
	...
	methods (Access = protected, Hidden)
		...
		function dispatchRemoteLoad(obj)
			...
			numPerBlock = obj.NumFramesPerBufferBlock;
			parallelPoolObj = obj.EnvironmentObj.ParallelPoolObj;
		...
		function varargout = makeTiffObj(obj)
			...
			ignition.shared.io.tiff.suppressTiffWarnings()
	...
	methods (Hidden)
		function initialize(obj)
			...
			% START PARALLEL POOL
			obj.initialize@ignition.core.Module(); % currently empty
			
			
			% IMPORT LOCAL NAMESPACE FOR EXTERNAL FUNCTIONS
			% 			import([obj.SubPackageName, '.*']);
			import ignition.shared.io.tiff.*

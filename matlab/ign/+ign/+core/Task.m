...
% stk = parallel.internal.datastructs.Stack
	...
	properties (SetAccess = protected)%immutable)
		Input ign.core.TaskInput
		Output ign.core.TaskOutput
	end
	...
	methods
		...
		function obj = Task(fcn, varargin)
			% ign.core.Task - An Operation with specified I/O Dependencies
			% Usage:
			%			>> obj = ign.core.Task( operationHandle );
			%

			% CALL SUPERCLASS CONSTRUCTOR
			obj = obj@ign.core.Operation(fcn);
		...
		function initializeTaskInput(obj)
			...
			while k<obj.NumInputArguments
				k=k+1;
				obj.Input(k) = ign.core.TaskInput(obj,k);
			end
		end
		function initializeTaskOutput(obj)
			...
			while k<obj.NumOutputArguments
				k=k+1;
				obj.Output(k) = ign.core.TaskOutput(obj,k);
			end
		end
		...
		function receiveInputFromProp(obj, srcObj, srcPropName, inputIdx)
			...
			if (numProp == numIdx)
				for k = 1:numProp
					name = srcPropName{k};
					val = srcObj.(name);
					propSource = ign.core.TaskProperty( srcObj, name, val);
					idx = inputIdx(k);
				...
				% todo
				srcData = ign.core.TaskData( srcObj, srcPropName, getStructFromProps(srcObj,srcPropName));

classdef (CaseInsensitiveProperties, TruncatedProperties) ...
	...
	properties (SetAccess=protected)
		Source ign.core.tasks.TaskIO
		Target ign.core.tasks.TaskIO
		DependentTaskList ign.core.Task %todo
	end
	...
	methods
		...
		function tasklink = link( ioA, ioB)
			...
			assert( isa(ioA,'ign.core.tasks.TaskIO') && isa(ioB,'ign.core.tasks.TaskIO'))

			% DETERMINE DIRECTION OF INTENDED DATAFLOW LINKAGE
			if isa(ioA, 'ign.core.TaskOutput')
				ioSource = ioA;
				ioTarget = ioB;
			elseif isa(ioA, 'ign.core.TaskInput')
				ioTarget = ioA;
				ioSource = ioB;
			else
				if isa(ioB, 'ign.core.TaskOutput')
					ioSource = ioB;
					ioTarget = ioA;
				elseif isa(ioB, 'ign.core.TaskInput')
					ioSource = ioA;
					ioTarget = ioB;
				else
					warning('Ign:TaskIO:link:LinkDirectionUnspecified',...
			...
			if ~isscalar(ioTarget)
				...
				tasklink = ign.core.tasks.TaskLink( ioSource, ioTarget );

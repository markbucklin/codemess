classdef (CaseInsensitiveProperties, TruncatedProperties)  BufferedTiffStackLoader <  ignition.system.SystemInterface ...
		& matlab.system.mixin.FiniteSource
	% BufferedTiffStackLoader -> ThreadedImageLoader (TODO)
	...
	properties (Nontunable, Logical)
		...
		FrameObjectOutputPort = false % todo
	end
	...
	properties (SetAccess = ?ignition.system.SystemInterface, Nontunable, Hidden)
		ParallelFutureObj
		NumBufferBlocks
		PoolTimeoutListener
	end
	...
	properties (SetAccess = ?ignition.system.SystemInterface, Hidden)
		StartFrameIdx = 1
	...
	methods
		function obj = BufferedTiffStackLoader(varargin)
			...
			obj = obj@ignition.system.SystemInterface(varargin{:});
	...
	methods (Access = protected)
		...
		function setupImpl(obj)
			...
			if ~obj.IsInitialized
				systemInit(obj); % @SystemInterface
				initialize(obj)
			end
		...
		function varargout = stepImpl(obj)
			...
			if (obj.UseParallel) && (~obj.EnvironmentObj.ParallelPoolObj.Connected)
				fprintf('BufferedTiffStackLoader: pool disconnect (todo)\n')
	...
	methods (Hidden)
		function initialize(obj)
			...
			% START PARALLEL POOL
			obj.initialize@ignition.system.SystemInterface(); % currently empty

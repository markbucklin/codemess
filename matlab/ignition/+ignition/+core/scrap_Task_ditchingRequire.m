classdef (CaseInsensitiveProperties, TruncatedProperties) ...
	...
	methods
		...
		function obj = Task(fcn, varargin)
			...
			% obj = obj@ignition.core.Operation(fcn, varargin{:});
			% may need
			%			superclassArgsIn = [{fcn} , varargin];
			%			obj = obj@ignition.core.Operation(superclassArgsIn{:});
		...
		function require(obj, requiredTaskObj, whatIsRequired, inputDest)
			...
			if isnumeric(whatIsRequired)
				requireOutputFrom(obj, requiredTaskObj, whatIsRequired, inputDest)

			else
				requirePropFrom(obj, requiredTaskObj, whatIsRequired, inputDest)
			end

		end
		function requireOutputFrom(obj, requiredTaskObj, outputIdx, inputDest)
			...
			requiredTaskObj.NumOutputArguments = max(...
				requiredTaskObj.NumOutputArguments, max(outputIdx(:)));
			...
			%	obj, inputIdx, requiredTaskObj, outputIdx);
			dependencyObj = ignition.core.Dependency.buildDependency(...
				obj, inputDest, requiredTaskObj, outputIdx);

			% ADD HANDLES TO DEPENDENT & REQUIRED TASK-OBJECTS
			addDependency(obj, dependencyObj, inputDest);
			registerOutputDependency(requiredTaskObj, dependencyObj);

		end
		function requirePropFrom(obj, requiredTaskObj, requiredProp, inputDest)
		%function bindDependentPropertySource(obj, requiredTaskObj, requiredProp, inputDest)%
			% rename StateVar?
			...
			% CONSTRUCT DEPENDENCY OBJECT
			dependencyObj = ignition.core.Dependency.buildDependency(...
				obj, inputDest, requiredTaskObj, requiredProp);
		...
		function requireOutputFromAny(obj, requiredTaskObjList, outputIdx, inputIdx)
			% IF INPUT/OUTPUT ARGUMENT INDICES NOT SPECIFIED -> DEFAULT TO 1
			numReqTasks = numel(requiredTaskObjList);
			...
			% UPDATE NUM-OUTPUT-ARGS FOR REQUIRED-TASK-OBJECT
			newNumOut = cellfun( @max, {requiredTaskObjList.NumOutputArguments}, ...
				cellfun(@max, outputIdx, 'UniformOutput', false), 'UniformOutput', false);
			[requiredTaskObjList.NumOutputArguments] = newNumOut{:};

			% CONSTRUCT DEPENDENCY OBJECT
			dependencyObj = ignition.core.InputDependency(...
				obj, inputIdx, requiredTaskObjList, outputIdx);

			% ADD HANDLES TO DEPENDENT & REQUIRED TASK-OBJECTS
			addInputDependency(obj, dependencyObj, inputIdx);

			for kSrc = 1:numReqTasks
				reqTask = requiredTaskObjList(kSrc);
		...
		function bindOutputTarget(obj, targetTaskObj, targetDest, outputIdx)

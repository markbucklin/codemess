classdef (CaseInsensitiveProperties, TruncatedProperties) ...
	...
	properties (SetAccess = protected)		
		ElementBlock @ignition.core.FrameBufferElement vector
		ElementFrameCount
		MaxNumFramesPerUnit
	...
	methods
		function obj = FrameBuffer(frameCapacity, useFixed)
			...
			%
			%		Fixed FrameCapacity:
			%				>> obj = ignition.core.FrameBuffer(32, true)
			%				>> obj = ignition.core.FrameBuffer(inf)
			%
			%		Variable FrameCapacity:
			%				>> obj = ignition.core.FrameBuffer(32, false)
			%				>> obj = ignition.core.FrameBuffer(32)
			%				>> obj = ignition.core.FrameBuffer()
			%
			...
			obj.ElementBlock(1, initialFrameCapacity) = ignition.core.FrameBufferElement();
	...
	methods
		function write(obj, data)
			...
			% NUMERIC ARRAY INPUT			
			% [numDataChannels, numDataFrames] = ignition.shared.getNumChannelsAndFrames(data);
			...
			if (emptyCount < 0)
				if (~obj.IsFixedCapacity)
					...
					warning('Ignition:Core:FrameBuffer:Write:FrameBufferOverWrite',...
	...
	methods (Hidden)
		function frameCap = extendCapacity(obj, fillCount)
			...
			if (lastWriteIdx > obj.LastReadIdx)
				% POST EXTEND
				obj.ElementBlock(1,frameCap) = ignition.core.FrameBufferElement();
			else
				% INSERT EXTENSION
				extensionInsertElements = repelem(ignition.core.FrameBufferElement(), 1, extensionCount);
	...
	methods (Static)
		function obj = buildRingBuffer(capacity)
			...
			obj = ignition.core.FrameBuffer( capacity, true);
		...
		function obj = buildExpandableBuffer( initialCapacity )
			...
			obj = ignition.core.FrameBuffer( initialCapacity, false);

...
% 		FrameSize
	...
	methods (Access = protected, Hidden)
		function varargout = fillNextBufferBlock(obj)
			...
			nextFutureObj = obj.ParallelFutureObj(bufferIdx);
			try				
				[~, frameData, frameTime, frameInfo] = fetchNext(nextFutureObj);%, .5);
				
			catch
				% (~nextFutureObj.Read) && ...
				if any(strcmp(nextFutureObj.State, {'queued','running'}))
					% IF QUEUED OR RUNNING -> WAIT A MAX OF 2 SECONDS
					wait(nextFutureObj, 2)
						[~, frameData, frameTime, frameInfo] = fetchNext(nextFutureObj);%, .5);
				end
	...
	methods
		...
		function varargout = run(obj)
			...
			streamOut = ignition.core.type.VideoFrame(frameData, frameInfo, frameTime, frameIdx);
% 			streamOut = ignition.core.type.FrameData.splitFromOrderedInput(frameData, frameTime, frameInfo, frameIdx);
